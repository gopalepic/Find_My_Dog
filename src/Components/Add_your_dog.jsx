  ...
  return (
    <div>
       <div className='rounded-lg shadow-md bg-[#65350F] '>
          ...
          <h1 className='text-2xl'> Breed:  <input type="text"  onChange={handleSearch}
          className=' bg-[#b38b6d] rounded-xl m-2 cursor:pointer'/>
             <ul className='bg-[#4B371C] w-1/5 rounded-xl '>
              {filteredBreeds.map((breed , index) => (
                <li key={index} >{<button onClick={() => selectionOfBreed(breed)} className='hover:bg-[#FFE4C4]'>{breed}</button>}</li>
              )
              )}
             </ul>
          </h1>
